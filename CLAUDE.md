...
It serves on **4319**, and on Windows that number is a coin flip: Hyper-V reserves scattered TCP ranges, and on this dev machine 4319 falls inside the reserved `4222-4321`, so the web server dies with `EACCES` before a single test runs — a red run that says nothing about the code. Check with `netsh interface ipv4 show excludedportrange protocol=tcp`, then pick a port outside every listed range and run `E2E_PORT=9317 pnpm test:e2e`. The override reaches the server only because `test:e2e` declares `passThroughEnv: ["E2E_PORT"]` in `turbo.json` — turbo filters undeclared env vars silently, so setting the variable without that entry looks like the override is broken when it never arrived. CI stays on the 4319 default.
