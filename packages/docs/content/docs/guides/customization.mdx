...
// Input with a custom focus indicator.
// Three things matter here. The kit draws focus with an OUTLINE, not a ring —
// a `focus-visible:ring-*` class will not replace it, it will paint a second
// indicator alongside. The kit styles focus with `focus-visible:`, so a
// `focus:` class does not conflict with it and both would apply. And the
// indicator is a light/dark PAIR — tailwind-merge keys on the modifier set, so
// replacing only the light half leaves the dark half in place and the outline
// stays steel in dark mode. Override both halves.
<Input className="focus-visible:outline-purple-500 dark:focus-visible:outline-purple-500 focus-visible:border-purple-500 dark:focus-visible:border-purple-500" />
...
// Result: 'text-red-500 p-8'
// tailwind-merge resolves p-4 vs p-8 → keeps p-8.
// Surviving classes stay in their original order.
...
// Elevation tokens override cleanly too
cn('shadow-soft', 'shadow-lg')
// Result: 'shadow-lg'
```

Nim UI's `cn()` is configured with the kit's `shadow-soft` / `shadow-panel` / `shadow-control`
elevation tokens. If you build your own `cn()` with a bare `twMerge`, tailwind-merge reads those
tokens as shadow *colors* — `shadow-soft shadow-red-500` collapses to `shadow-red-500` and the
elevation is lost. Import `cn` from `@nim-ui/components` rather than rolling your own.
