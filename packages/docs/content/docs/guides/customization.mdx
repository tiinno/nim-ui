...
// Two things matter here. The kit styles focus with `focus-visible:`, so a
// `focus:` class does not conflict with it and both would apply. And the kit's
// focus indicator is a light/dark PAIR — tailwind-merge keys on the modifier
// set, so replacing only the light half leaves the dark half in place and the
// ring stays steel in dark mode. Override both halves.
<Input className="focus-visible:ring-purple-500 dark:focus-visible:ring-purple-500 focus-visible:border-purple-500 dark:focus-visible:border-purple-500" />
