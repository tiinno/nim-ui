...
### 2. A global animation damper in the token stylesheet

`tokens.css` also ships a blanket reset, so any animation the kit does not own -- including one you add in your own app -- is reduced to an imperceptible tick:

```css
/* Applied internally by Nim UI (packages/ui/src/tokens.css) */
