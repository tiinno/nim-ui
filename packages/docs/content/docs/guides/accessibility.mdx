...
Nim UI honours `prefers-reduced-motion` automatically. There is **no prop, no provider, and no consumer code** to write: import the stylesheet and every entrance and exit animation in the kit switches itself off for users who have asked their OS for reduced motion.
...
Loading indicators are excluded from *this* per-component treatment. `Spinner`, `Skeleton`, `Dot`, `StatusPill`, and `Button`'s loading state keep their `animate-spin` / `animate-pulse` class, because switching a loading indicator off outright removes the only signal that work is in flight. What actually reaches them under reduced motion is decided by the global damper below.

### 2. A global animation damper in the kit's stylesheet

`reduced-motion.css`, which the compiled `@nim-ui/components/styles` bundle imports, ships a blanket reset, so any animation the kit does not own -- including one you add in your own app -- is reduced to an imperceptible tick:

```css
/* Applied internally by Nim UI (packages/ui/src/reduced-motion.css) */
...
Note how far that rule reaches. It is `!important` and unlayered, so it outranks every utility: under reduced motion, `animate-spin` and `animate-pulse` run a single 0.01ms iteration and stop, and every `transition-*` in your app -- Nim's or your own -- collapses to 0.01ms. That is the current behaviour of a deliberately blunt reset, not a per-component design decision, and the treatment of loading indicators is under review. If you need a spinner to keep turning for reduced-motion users, re-declare it in your own stylesheet.
