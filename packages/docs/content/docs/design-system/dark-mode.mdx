---
title: Dark Mode
description: Enable and customize dark mode across all Nim UI components
---
...
<ComponentPreview title="Custom Component with Dark Mode">
  <div className="w-full rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-6">
    <h3 className="text-lg font-semibold text-neutral-900 dark:text-neutral-100 mb-2">Custom Card</h3>
    <p className="text-neutral-600 dark:text-neutral-400 mb-4">This card adapts its colors based on the current theme mode.</p>
    <div className="flex gap-2">
      <span className="px-2 py-1 text-xs rounded bg-primary-100 dark:bg-primary-900 text-primary-700 dark:text-primary-300">Tag</span>
      <span className="px-2 py-1 text-xs rounded bg-neutral-100 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300">Label</span>
    </div>
  </div>
...
Tailwind v4 has no `darkMode` setting and no `content` array — a `tailwind.config.js` isn't read at all unless you point at it explicitly with `@config`, and one sitting unused does nothing ([Configuration](/getting-started/configuration/) covers this in full). The `dark:` variant this page has been toggling is bound by one declaration, already compiled into the stylesheet you import:

```css
@custom-variant dark (&:where(.dark, .dark *, [data-theme="dark"], [data-theme="dark"] *));
```

This line is declared in `packages/ui/src/styles.css` and ships inside `@import '@nim-ui/components/styles'` — importing that one file, as shown throughout this page, is the entire setup. It matches more than the `.dark` class: an element under a `[data-theme="dark"]` attribute activates `dark:` utilities too, a hook for frameworks that set a `data-theme` attribute instead of a class (this documentation site's own Fumadocs toggle only ever sets `.dark`).

The binding also isn't something your own stylesheet can change. `@import '@nim-ui/components/styles'` brings in an already-compiled bundle in which `@custom-variant` has already been expanded into concrete `:where(...)` selectors on every rule, so redeclaring `@custom-variant dark` yourself — before or after that import — has no effect, and there is no `darkMode` setting to flip instead. See [Theming → Dark mode](/guides/theming/#dark-mode) for the full mechanism, including why a single OKLCH value per shade covers both themes.
