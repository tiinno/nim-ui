...
DatePicker composes [Popover](/components/overlay/popover/), [Calendar](/components/forms/calendar/), and a button-styled trigger into a ready-to-use date input. Two variants ship: `DatePicker` for a single date and `DateRangePicker` for a start–end range with a two-month preview.
...
- [Popover](/components/overlay/popover/) — the underlying floating layer
