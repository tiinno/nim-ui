...
Tiinno UI components are designed to be customized without fighting the framework. Every component accepts a `className` prop for style overrides, uses class-variance-authority (CVA) for variant management, and provides the `cn()` utility for merging Tailwind classes safely.
...
```tsx
import { Card } from '@tiinno-ui/components';
...
Tiinno UI uses the `cn()` utility internally for class merging. It combines `clsx` for conditional classes with `tailwind-merge` for resolving Tailwind class conflicts.

### How It Works

```tsx
import { cn } from '@tiinno-ui/components';
...
When building your own components that wrap Tiinno UI, use `cn()` to merge default and custom classes safely:

```tsx
import { cn } from '@tiinno-ui/components';
import { Button, type ButtonProps } from '@tiinno-ui/components';
...
Tiinno UI uses class-variance-authority (CVA) for managing component variants. You can extend the variant system to add your own custom variants.
...
Wrap a Tiinno UI component and add custom variants:

```tsx
import { forwardRef } from 'react';
import { cva, type VariantProps } from 'class-variance-authority';
import { cn } from '@tiinno-ui/components';
import { Button as BaseButton, type ButtonProps as BaseButtonProps } from '@tiinno-ui/components';
...
import { cn } from '@tiinno-ui/components';
import { Button as BaseButton } from '@tiinno-ui/components';
...
Build higher-level components by composing Tiinno UI primitives:

### Stat Card

```tsx
import { Card, Badge } from '@tiinno-ui/components';
import { cn } from '@tiinno-ui/components';
...
- [Best Practices](/guides/best-practices/) - Tips for building with Tiinno UI
